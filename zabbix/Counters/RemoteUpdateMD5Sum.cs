...
namespace ZabbixCounters.Counters
{
	...
	public class RemoteUpdateMD5Sum : ILoadableCounter
	{
		...
		{ 
			...
			if (key.StartsWith("agent.update.md5")) 
			{
				Configuration.SetObjectInHashByKey("UpdatePackageMD5Sum", key.Remove(0,key.IndexOf("[",0, key.Length)).Trim('[').Trim(']'));
				return true;
			} 
			return false;
		}
